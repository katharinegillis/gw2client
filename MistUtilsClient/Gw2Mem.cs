...
namespace Gw2Mem
{
    // From: https://forum-en.guildwars2.com/forum/community/api/Map-API-Mumble-Mashup/first
    // I made some adjustments to use my Avatar class to store the information in.
    ...
    class MumbleLink : IDisposable
    {
        ...
        {
            public MumbleLink.Coordinate Coordinates;
            public string Name;
        ...
        public PlayerInfo GetPlayerInfo()
        {
            ...
            playerInfo.Coordinates = coord;
            if (l.identity != null && l.identity != "")
            {
                dynamic identity = JsonConvert.DeserializeObject<dynamic>(l.identity);
                playerInfo.Name = identity.name;
                playerInfo.Profession = identity.profession;
                playerInfo.TeamColourId = identity.team_color_id;
                playerInfo.Commander = identity.commander;
            }
            else
            {
                playerInfo.Name = "";
